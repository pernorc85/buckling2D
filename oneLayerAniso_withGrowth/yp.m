...
sigma1=1/alpha*1/alpha;   %=alpha*W1*dI1/dalpha-1;
...
%calculate pressure by integration
%alpha=r/R/g2;
%W'(alpha) = W1*dI1/d_alpha + W4*dI4/d_alpha + W6*dI6/d_alpha
%          = mu*(alpha^(-3)+alpha)
%           +W4*((sin(gamma))^2*(-2)*alpha^(-3) + (cos(gamma))^2*2*alpha)
%           +W6*((sin(gamma))^2*(-2)*alpha^(-3) + (cos(gamma))^2*2*alpha;
%alpha/r*W'(alpha)
